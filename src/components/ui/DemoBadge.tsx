...
export function DemoBadge({ endpoint, className = "" }: { endpoint: string; className?: string }) {
  ...
  return (
    <span
      ...
      className={`inline-flex shrink-0 items-center gap-1 rounded-full border border-amber-300 bg-amber-50 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-amber-700 dark:border-amber-700/50 dark:bg-amber-900/30 dark:text-amber-400 ${className}`}
    >
